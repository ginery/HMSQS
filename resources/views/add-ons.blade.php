            ...
            <div class="intro-y col-span-6 sm:col-span-4 md:col-span-3 xxl:col-span-2" >
                <div class="file box rounded-md px-5 pt-8 pb-5 px-3 sm:px-5 relative zoom-in">
                    ...
                    <div class="text-center mt-3 d-flex justify-center"> 
                        <div>Qty:</div> 
                        <input type="number" id="qty-{{$service->id}}" min="0" class="input w-20 border mt-2" value="1" disabled>
                        <div class="mt-3 ml-3">
                            <input id="cb_{{$service->id}}" class="input border border-gray-500 room-checkbox" value="{{$service->id}}" type="checkbox" onchange="is_selected({{$service->id}})">
                        </div>
                    </div>
                </div>
            </div>
        ...
        if(service_data.length > 0){
            if(confirm("Are you sure to add service(s) into your room?")){
                $.ajax({
                    ...
                    type: 'POST',
                    data: {service_data: service_data, reservation_id: reservation_id, user_id: user_id},
                    success: function(response) {
                        if(response > 0){
